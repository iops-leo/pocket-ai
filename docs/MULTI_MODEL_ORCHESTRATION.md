...
### 3. 토큰 최적화 및 지연 시간 방어 (Fast-track)
서브 워커를 무조건 매 대화 턴(Turn)마다 호출하면 2~3개 모델의 API 토큰 비용이 중복 발생하고 응답 시간이 지연됩니다.
따라서 Pocket AI의 오케스트레이션은 철저한 **비용/속도 최적화 구조**를 따릅니다:
- **단순 변수명 변경, 오타 수정** 등 직관적인 요청은 Claude 본인이 즉시 처리(Fast-track)하여 서브 워커 호출을 생략합니다.
- UI 설계 구조화, 방대한 아키텍처 재설계 등 **거대 컨텍스트가 필요한 경우에만** Claude가 도구의 Description(설명서)을 읽고 "이 작업은 제미나이/코덱스에게 넘기는 것이 내 토큰을 아끼면서 좋은 결과물을 낼 수 있겠다"라고 스스로 판단할 때만 워커를 스폰합니다.
