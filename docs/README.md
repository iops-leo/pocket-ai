...
### 서버 Blind Relay
- 서버는 암호화된 메시지만 중계
- 메타데이터(sessionId, timestamp)만 확인
- 사용자 데이터 접근 불가
...
### 비용 최적화
- 초기: Fly.io + PostgreSQL ($5-15/월)
- 성장: 스케일업 PostgreSQL ($50-150/월)
- 대규모: 멀티리전 ($300+/월)
...
pocket-ai/
├── apps/
│   ├── server/      # Fastify + Socket.IO 릴레이 서버 + Prisma + PostgreSQL
...
| Server | Fastify + Socket.IO | Fly.io ($5-15) |
| Database | PostgreSQL + Prisma ORM | Fly.io / Supabase |
