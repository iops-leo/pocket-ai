...
import { IPCServer, type IPCMessage, type IPCResponse } from './ipc-server.js';
...
export class Daemon {
   ...
   * IPC 메시지 핸들러
   */
  private async handleIPCMessage(message: IPCMessage): Promise<IPCResponse> {
    try {
      switch (message.type) {
        case 'start-session':
   ...
   * 세션 시작
   */
  private async handleStartSession(payload: { cwd: string; engine: string }): Promise<IPCResponse> {
   ...
   * 세션 중지
   */
  private async handleStopSession(payload: { sessionId: string }): Promise<IPCResponse> {
   ...
   * 세션 전환
   */
  private async handleSwitchSession(payload: { sessionId: string }): Promise<IPCResponse> {
    const { sessionId } = payload;
   ...
   * 세션 목록
   */
  private async handleListSessions(): Promise<IPCResponse> {
